...
static const double inactiveopacity =
    1.0f; /* Window opacity when it's inactive (0 <= opacity <= 1) */
...
static const int nviews =
    0; /* mask of tags highlighted by default (tags 1-4) */
static const int statusall =
    1; /* 1 means status is shown in all bars, not just active monitor */
...
#define BARTAB_TAGSINDICATOR                                                   \
  1                     // 0 = off, 1 = on if >1 client/view tag, 2 = always on
#define BARTAB_TAGSPX 5 // # pixels for tag grid boxes
#define BARTAB_TAGSROWS 3 // # rows in tag grid (9 tags, e.g. 3x3)
static void (*bartabmonfns[])(Monitor *) = {monocle /* , customlayoutfn */};
static void (*bartabfloatfns[])(Monitor *) = {NULL /* , customlayoutfn */};

static const float facts[1]; // static const float facts[]     = {     0, 0.5 };
                             // // = mfact   // 50%
static const int
    masters[1]; // static const int masters[]     = {     0,      -1 }; // =
                // nmaster // vertical stacking (for rotated monitor)
static const int views[1]; // static const int views[]       = {     0,      ~0
                           // }; // = nviews  // all tags
/* invert tags after nviews */
/* array dimentions can both be as big as needed */ // final highlighted tags
static const int
    toggles[1][1]; // static const int toggles[2][2] = { {0,8}, {~0,~0} }; //
                   // 2-4+9     // all (leave as views above)
...
static const Key keys[] = {
    ...
    {WINKEY,                       XK_e,      setlayout,      {.v = &layouts[4]} },
    {WINKEY|ShiftMask,             XK_e,      setlayout,      {.v = &layouts[5]} },
    ...
    TAGKEYS(XK_1, 0) TAGKEYS(XK_2, 1) TAGKEYS(XK_3, 2) TAGKEYS(XK_4, 3)
        TAGKEYS(XK_5, 4) TAGKEYS(XK_6, 5) TAGKEYS(XK_7, 6) TAGKEYS(XK_8, 7)
            TAGKEYS(XK_9, 8){WINKEY, XK_grave, reset_view, {0}},
